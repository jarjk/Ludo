 ...
 * does, that'd require many many tests which I don't want to perform.
 * I'd like to have some kind of smooth_out.
 * The pretty_out is now capable of printing houses. A bit more beautiful than
 * in the last version. Also a bit cleaner code. Bug found: the house can go
 * wrong by not going into the last field, instead clearing another one.
 * Something could be wrong with knock_out and idx_house. BUT! it doesn't happen
...
// game board: nothing: _ = 0, Blue: 1, Red: 2, Green: 3, Yellow: 4
...
struct colour {
    ...
    int knock_out_num{};
    /* the start field on the board*/
    int start{};
    /* the field on which they last step before going to the waiting board or
     * house.*/
    int end{};
...
bool is_on_board(const int &color_num);
...
/* print the waiting board for pretty_out */
...
void round() {
    ...
    /* {
        ...
        // board is empty
        if (!is_on_board(blue.colour_num)) {
...
bool is_on_board(const int &color_num) {
    int idx = 0;
    while (board[idx] != color_num && idx < 52)
        idx++;
    return idx < 52;
}
