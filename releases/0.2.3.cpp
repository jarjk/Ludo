 ...
 * display stuff in the way people like it. It's very basic, for example the
 * houses aren't at all displayed but I'm working on it. In my opinion the code
 * is quite great, thanks to clangd. In the next few versions i'll make the
 * pretty_out look even better I think. After that, the code should be made more
 * clear and easy to understand, no unneeded commented old not working stuff. I
 * have functions for most of the stuff which is pretty great. And after some
 * time I'll have to work on stuff like smooth_out. It would basically step on
 * every field instead of jumping a huge one.
 */
